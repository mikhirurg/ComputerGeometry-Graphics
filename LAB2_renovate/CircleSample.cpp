...
int main() {
    try {
        ...
        for (double deg = 0; deg < 360; deg+=1.0) {
            ...
            double x1 = x0 + len * cos(deg*M_PI/180);
            double y1 = y0 - len * sin(deg*M_PI/180);
            img.drawLine(255, 50, x0, y0, x1, y1, gamma);
            img.writeImg("img/out"+std::to_string((int)deg)+".pgm");
            std::system(("magick convert img/out"+std::to_string((int)deg)+".pgm img/out"+std::to_string((int)deg)+".png").c_str());
            std::cout<<std::to_string(deg)<<std::endl;
        }
        std::system("magick convert img/out{0..360}.png convert img/out.gif");
